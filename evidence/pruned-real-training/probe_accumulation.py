...
def main() -> int:
    ...
    out = Path(rf"F:\llm-models\_a4b\probe-accumulation-{label}.json")
    ...
    rows = [json.loads(l) for l in Path(EVAL).read_text(encoding="utf-8").splitlines() if l.strip()][:N_PROBLEMS]
    ...
    with torch.inference_mode():
        for i, row in enumerate(rows):
            ...
            snap.update({
                "problem": i, "new_tokens": new_tokens, "seconds": seconds,
            ...
            steps.append(snap)
            log(f"    problem {i:>2} {new_tokens:>4} tok  {snap['ms_per_token']:6.1f} ms/tok  "
                f"reserved {snap['torch_reserved_gib']:5.2f}  "
                f"non-torch {snap['non_torch_gib']:5.2f} "
                f"(+{snap['non_torch_above_load_gib']:.2f})  "
