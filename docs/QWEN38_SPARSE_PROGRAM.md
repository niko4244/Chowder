...
3. **Protected evaluation suite content does not exist yet — the
   ...
   hook, and FK-anchored persistence into `evaluation_runs`. The
   protected *content* now exists too:
   `src/chowder/parent_suite_content.py` (v1) authors 54 original
   hand-authored items — 6 per dimension, deterministic `expected`
   values, behavior/calibration probe-framed — materialized by
   `materialize_protected_suites` into datasets plus hash-only
   fingerprint indexes built through the canonical
   `parent_eval.build_protected_suite_dir` machinery, with a root-free
   manifest whose dataset/index sha256s make the holdout verifiable
   without exposing its text. The prior
...
- [ ] protected parent-evaluation suite established (harness in `src/chowder/parent_eval.py`; content authored in `src/chowder/parent_suite_content.py` v1 — 54 original items, 6 per dimension, hash-only indexes, contamination audit verified; the box stays unchecked until the suite has actually scored a real parent run)
...
evaluation runs (the suite content and both parents' on-disk weights
remove the two acquisition blockers that previously gated them — what
remains is B's full-mode manifest, parent C/D acquisition, and the
runs themselves).
