...
def test_repair_candidate_is_deterministic_and_does_not_patch_evaluation(tmp_path):
    ...
    first = build_repair_candidate(parent_id="root", plan=_plan(), dataset=dataset, variant=variant)
    second = build_repair_candidate(parent_id="root", plan=_plan(), dataset=dataset, variant=variant)
...
def test_population_requires_unique_variant_names(tmp_path):
    ...
    with pytest.raises(ValueError, match="names must be unique"):
        build_repair_population(
            ...
            plan=_plan(),
            dataset=dataset,
            variants=(RepairVariant("same", 0.2), RepairVariant("same", 0.3)),
        )
