...
Still ahead: FSDP for multi-GPU (DDP now supported), wiring the autonomous repair loop into the default single-command user path, and HF/model-download infrastructure resilience (retries, offline mode, preflight checks). See [`docs/ROADMAP.md`](docs/ROADMAP.md) for the full, currently-accurate list.
