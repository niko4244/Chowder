...
- **Offline/local-model mode and dependency preflight**: `backend.offline` (or `evaluation.offline`) skips the network entirely and loads only from the local Hugging Face cache — a cache miss fails immediately rather than retrying into a wait it was told never to attempt. Separately, a missing `torch`/`transformers`/`peft`/`datasets`/`accelerate`/`bitsandbytes` install is now caught during the cheap preflight step, before any GPU-hours are reserved or a worker subprocess is spawned, dispatched by the real executor type rather than by name so hand-written test doubles are never mistaken for it.
- **Disk-space and model-architecture preflight**: free space at `work_dir` is checked against a configurable `backend.min_free_disk_gb` (default 2 GB) before any GPU-hours are reserved, so an out-of-space failure is a clean config-time rejection instead of a crash partway through a model download or checkpoint write. Separately, `base_model`'s config is resolved (no weight download) and checked against `AutoModelForCausalLM`'s own architecture registry — pointing Chowder at a non-causal-LM repo (an encoder-only or encoder-decoder model, say) is rejected before paying for a subprocess and reservation rather than failing confusingly deep inside training.
- **Persistent cache-hit/miss reporting**: every model load (training and both evaluators) records whether the base model was already in the local Hugging Face cache or had to be fetched, in `model_provenance.model_cache_status` — visible in the same evidence trail as the resolved commit and package versions.
