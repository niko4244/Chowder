...
Still ahead: FSDP for multi-GPU (DDP now supported), auto-detected LoRA target modules and common architecture presets, hardware-aware recipe defaults, and wiring the autonomous repair loop into the default single-command user path. See [`docs/ROADMAP.md`](docs/ROADMAP.md) for the full, currently-accurate list.
